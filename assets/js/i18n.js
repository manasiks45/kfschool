(function () {
  ...
  const routeMap = {
    ...
    "year-calendar.html": "year-calendar-ar.html",
    "announcements.html": "announcements-ar.html",
  };
  function getLang() {
    const s = localStorage.getItem(LS_KEY);
    return s === "en" || s === "ar" ? s : DEFAULT_LANG;
  }
  function setLang(l) {
  ...
  function fileName(url) {
    ...
    const a = p.split("/");
    return a[a.length - 1] || "index.html";
  }
  function englishBase(f) {
    return f.endsWith("-ar.html") ? f.replace("-ar.html", ".html") : f;
  }
  function twin(f, l) {
    const b = englishBase(f);
    const t = routeMap[b];
    if (!t) return f;
    return l === "ar" ? t : b;
  }
  function redirectIfMismatch(l) {
    const cur = fileName(window.location.pathname);
    const want = twin(cur, l);
    if (cur !== want) {
  ...
  function rewriteLinks(l) {
    document.querySelectorAll("a[href]").forEach((a) => {
      ...
      if (
        ...
        return;
      const f = fileName(href);
      const b = englishBase(f);
      if (routeMap[b]) a.setAttribute("href", twin(f, l));
    });
  }
  function applyDir(l) {
  ...
  function init() {
    ...
    if (en)
      en.addEventListener("click", (e) => {
        e.preventDefault();
        setLang("en");
        redirectIfMismatch("en") || (applyDir("en"), rewriteLinks("en"));
      });
    if (ar)
      ar.addEventListener("click", (e) => {
        e.preventDefault();
        setLang("ar");
        redirectIfMismatch("ar") || (applyDir("ar"), rewriteLinks("ar"));
      });
  }
